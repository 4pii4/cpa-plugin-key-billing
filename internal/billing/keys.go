...
func (s *Store) BindKey(scope, planID string) error {
	...
	if scope == "" || planID == "" {
		return invalidf("API key scope and subscription plan ID cannot be empty")
	}
...
func (s *Store) ResetQuota(req ResetRequest) (ResetResult, error) {
	...
	if req.Mode == "global" {
		if len(req.Scopes) != 0 {
			return ResetResult{}, invalidf("A global reset cannot specify API keys")
		}
	} else if req.Mode == "all" {
		if len(req.Scopes) == 0 {
			return ResetResult{}, invalidf("Select at least one API key to reset")
		}
	} else {
	...
	return editConfiguration(s, func(state *State) (ResetResult, Changes, error) {
		...
		for _, scope := range scopes {
			key := state.liveKey(scope)
			if key == nil || key.PlanID == "" {
				return ResetResult{}, Changes{}, invalidf("API key does not exist or has no subscription plan")
			}
		}
		result := ResetResult{}
		var changed []string
...
func (s *Store) SyncKeys(keys []string, allowEmpty bool) (SyncResult, error) {
	...
	if len(scopes) == 0 && !allowEmpty {
		return SyncResult{}, invalidf("API key list is empty; pass allow_empty to clear it")
	}
