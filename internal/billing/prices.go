...
func (s *Store) ModelPriceRows(models []string, includeCustom bool) ([]PriceRow, error) {
	...
	for _, name := range names {
		...
		if len(name) > 1024 {
			return nil, invalidf("Model ID too long")
		}
		seen[key] = true
...
func (price CustomPrice) Validate() error {
	...
	if modelID == "" {
		return invalidf("Model ID cannot be empty")
	}
	if len(modelID) > 1024 {
		return invalidf("Model ID too long")
	}
...
func (s *Store) ResolveModelPrice(upstream, requested string, refresh bool) (Price, string, error) {
	...
	if switched {
		return Price{Source: PriceSourceNone}, model, fmt.Errorf("reference-price database changed; try again")
	}
