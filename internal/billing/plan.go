...
func (p Plan) Validate() error {
	if strings.TrimSpace(p.ID) == "" {
		return invalidf("Subscription plan ID cannot be empty")
	}
	if len(p.Windows) == 0 {
		return invalidf("Subscription plan must have at least one quota window")
	}
	...
	for _, window := range p.Windows {
		if window.ID == "" || ids[window.ID] {
			return invalidf("Quota-window ID is invalid or duplicated")
		}
		name := strings.TrimSpace(window.Name)
		if name == "" || len(name) > maxRouteNameBytes {
			return invalidf("Quota-window name cannot be empty or exceed %d bytes", maxRouteNameBytes)
		}
		if names[strings.ToLower(name)] {
			return invalidf("Quota-window name %q is duplicated", name)
		}
		if window.AmountUSD < 0 || math.IsNaN(window.AmountUSD) || math.IsInf(window.AmountUSD, 0) {
			return invalidf("Window %q: spending limit must be a non-negative finite number", name)
		}
		...
		if window.AmountUSD == 0 && window.TokenLimit == 0 && window.RequestLimit == 0 {
			return invalidf("Window %q: configure at least one limit", name)
		}
		...
		if window.CycleAnchorAt.IsZero() != p.Windows[0].CycleAnchorAt.IsZero() {
			return invalidf("All quota windows in a subscription plan must use the same cycle mode")
		}
		if !window.CycleAnchorAt.IsZero() && (window.CycleAnchorAt.Year() < 1970 || window.CycleAnchorAt.Year() > 9999 || window.CycleAnchorAt.Nanosecond() != 0) {
			return invalidf("Window %q: cycle start must be between 1970 and 9999 and precise to the second", name)
		}
	...
	for i := range windows {
		...
		if !window.CycleAnchorAt.IsZero() {
			...
			if oldIndex >= 0 && window.sameSchedule(existing[oldIndex]) {
				...
				return nil, invalidf("Window %q: next cycle start must be in the future and no more than one period away", window.Name)
			}
		}
	}
...
func (s *Store) UpdatePlanWithBindings(patch PlanPatch, scopes *[]string) (Plan, error) {
	...
	if patch.ID == "" {
		return Plan{}, invalidf("Subscription plan ID cannot be empty")
	}
...
func (s *Store) DeletePlan(id string) (int, error) {
	id = strings.TrimSpace(id)
	if id == "" {
		return 0, invalidf("Subscription plan ID cannot be empty")
	}
