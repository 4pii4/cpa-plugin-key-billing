...
func (r PriceRates) validate(modelID string) error {
	if invalidPrice(r.InputPer1M) || invalidPrice(r.OutputPer1M) {
		return invalidf("Model %q: input and output prices must be non-negative finite numbers", modelID)
	}
	if r.CacheReadPer1M != nil && invalidPrice(*r.CacheReadPer1M) {
		return invalidf("Model %q: cache-read price must be a non-negative finite number", modelID)
	}
	if r.CacheWritePer1M != nil && invalidPrice(*r.CacheWritePer1M) {
		return invalidf("Model %q: cache-write price must be a non-negative finite number", modelID)
	}
	if tier := r.LongContext; tier != nil {
		if tier.ThresholdInputTokens <= 0 {
			return invalidf("Model %q: long-context threshold must be greater than 0", modelID)
		}
		if invalidPrice(tier.InputPer1M) || invalidPrice(tier.OutputPer1M) {
			return invalidf("Model %q: long-context input and output prices must be non-negative finite numbers", modelID)
		}
		if tier.CacheReadPer1M != nil && invalidPrice(*tier.CacheReadPer1M) {
			return invalidf("Model %q: long-context cache-read price must be a non-negative finite number", modelID)
		}
		if tier.CacheWritePer1M != nil && invalidPrice(*tier.CacheWritePer1M) {
			return invalidf("Model %q: long-context cache-write price must be a non-negative finite number", modelID)
		}
	}
	return nil
}
