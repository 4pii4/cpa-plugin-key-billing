...
func (s *Store) Configure(cfg Config) error {
	...
	s.AddPluginLog(PluginLogInfo, "Billing database loaded: %s, %d API keys, %d subscription plans, %d request events, %s",
...
func editConfiguration[T any](s *Store, fn func(*State) (T, Changes, error)) (T, error) {
	...
	err := func() error {
		...
		if written {
			...
			if errSave != nil {
				return fmt.Errorf("failed to save configuration: %w", errSave)
			}
			s.dirty = Changes{}
		}
		s.state = &next
...
func (s *Store) recordWriteSuccess() {
	...
	s.errMu.Unlock()
	if recovered {
		s.AddPluginLog(PluginLogInfo, "Billing database write resumed")
	}
}
