...
func (a *App) validateNewCredentialRefs(refs, existing []string) *ManagementResponse {
	...
	if err := a.refreshCredentialInventory(); err != nil {
		response := JSONError(http.StatusBadGateway, "host_unavailable", "Failed to load upstream credentials: "+err.Error())
		return &response
	}
	if missing := a.missingCredentialRef(newRefs); missing != "" {
		response := JSONError(http.StatusBadRequest, "invalid", "Upstream credential no longer exists: "+missing)
		return &response
	}
	return nil
}
...
func (a *App) listPluginLogs(req ManagementRequest) ManagementResponse {
	...
	if raw := strings.TrimSpace(req.Query.Get("before_id")); raw != "" {
		...
		if err != nil || value < 1 {
			return JSONError(http.StatusBadRequest, "invalid", "Invalid paging cursor")
		}
	...
	if raw := strings.TrimSpace(req.Query.Get("since")); raw != "" {
		...
		if err != nil {
			return JSONError(http.StatusBadRequest, "invalid", "Start time must be in RFC3339 format")
		}
		query.Since = value
	}
...
func (a *App) syncKeys(req ManagementRequest) ManagementResponse {
	...
	if result.Added > 0 || result.Deleted > 0 {
		a.store.AddPluginLog(billing.PluginLogInfo, "CLIProxyAPI API keys synchronized: %d added, %d deleted",
...
func decodeStrict(body []byte, target any) error {
	...
	if errDecode := decoder.Decode(target); errDecode != nil {
		return &billing.Error{Kind: billing.KindInvalid, Msg: fmt.Sprintf("Invalid request body: %v", errDecode)}
	}
	if errTrailing := decoder.Decode(&struct{}{}); errTrailing != io.EOF {
		return &billing.Error{Kind: billing.KindInvalid, Msg: "Request body must contain exactly one JSON value"}
	}
	return nil
}
