	...
	if err := database.transact(func(tx *sql.Tx) error {
		...
		for _, table := range slices.Sorted(maps.Keys(indexes)) {
			if err := syncTableIndexes(tx, table, indexes[table]); err != nil {
				return fmt.Errorf("synchronize %s index: %w", table, err)
			}
		}
		return nil
	}); err != nil {
...
func (d *DB) initSchema(tx *sql.Tx) error {
	...
	case 0:
		...
		if err := tx.QueryRow(`SELECT count(*) FROM sqlite_master
			...
			return fmt.Errorf("inspect billing database %s: %w", d.path, err)
		}
		if existingTables != 0 {
			return fmt.Errorf("billing database %s uses an unsupported file format; configure a different state_file", d.path)
		}
	...
	if _, err := tx.Exec(fmt.Sprintf("PRAGMA user_version = %d", schemaVersion)); err != nil {
		return fmt.Errorf("mark billing database %s format version: %w", d.path, err)
	}
	return nil
}
...
func (d *DB) transact(fn func(*sql.Tx) error) error {
	tx, err := d.db.Begin()
	if err != nil {
		return fmt.Errorf("start billing database transaction: %w", err)
	}
