...
func callHost(method string, payload any) (json.RawMessage, error) {
	...
	if len(rawPayload) > 0 {
		...
		if request == nil {
			return nil, fmt.Errorf("allocate request memory for host call %s", method)
		}
	...
	if response.ptr != nil && response.len > 0 {
		if response.len > C.size_t(^uint32(0)>>1) {
			return nil, fmt.Errorf("host call %s response is too large", method)
		}
...
func cliproxyPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	...
	if request == nil && requestLen > 0 {
		writeResponse(response, plugin.ErrorEnvelope("invalid_request", "plugin request pointer is nil", http.StatusBadRequest))
		return 1
	}
	if request != nil && requestLen > 0 {
		length := C.int(requestLen)
		if length < 0 || C.size_t(length) != requestLen {
			writeResponse(response, plugin.ErrorEnvelope("invalid_request", "plugin request is too large", http.StatusBadRequest))
			return 1
		}
